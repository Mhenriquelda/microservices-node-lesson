...
### Criar usuário
```bash
curl -X POST http://localhost:3000/users -H "Content-Type: application/json" -d "{\"name\":\"Marcos\", \"email\":\"marcos@gmail.com\"}"
```
...
curl -X POST http://localhost:3000/orders -H "Content-Type: application/json" -d "{\"userId\":\"u_klfVBj\",\"items\":[{\"sku\":\"BOOK-123\",\"qty\":2}],\"total\":120.50}"
```
